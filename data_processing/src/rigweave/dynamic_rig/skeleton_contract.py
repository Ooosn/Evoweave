...
@dataclass(frozen=True)
class RowSkeletonContract:
    """Mapping from raw Blender bone rows to the pass1 training row table.

    This is intentionally conservative: it does not remove roots, compress
    connector chains, or build a target skeleton.  It only drops disposable
    unweighted tail/end leaf rows so that pass1 NPZ files keep real joints and
    row-aligned LBS arrays intact.
    """
    ...
    dropped_raw_indices: list[int]
    active_raw_indices: list[int]
    dropped_tail_leaf_indices: list[int]
...
def build_row_skeleton_contract(
    parents: np.ndarray | Sequence[int],
    skin_weights: np.ndarray,
    names: Sequence[str] | None = None,
    *,
    active_skin_threshold: float = 1.0e-4,
    drop_unweighted_tail_leaf: bool = True,
) -> RowSkeletonContract:
    """Build the cleaned pass1 row-skeleton table.

    Contract:
    - joint rows come only from explicit Blender bone rows, never from tail
      coordinates.
    - tail/end named rows are still real joint rows when they carry skin weight
      or connect descendants.
    - only unweighted tail/end leaf rows are dropped here.
    - root handling is left to strict/rootless stages.
    """
    ...
    children = children_from_parents(arr)
    drop: set[int] = set()
    if drop_unweighted_tail_leaf:
        for idx in range(n):
            if bool(active[idx]):
                continue
            if children[idx]:
                continue
            if looks_like_tail_or_end(name_list[idx]):
                drop.add(int(idx))

    raw_order = [idx for idx in range(n) if idx not in drop]
    if not raw_order:
        raise ValueError("row skeleton contract dropped every joint")
    old_to_new = {raw: new for new, raw in enumerate(raw_order)}
    new_parents = np.full((len(raw_order),), -1, dtype=np.int64)
    for raw in raw_order:
        p = int(arr[int(raw)])
        if p >= 0:
            if p not in old_to_new:
                raise ValueError(f"kept joint {raw} has dropped parent {p}")
            new_parents[old_to_new[int(raw)]] = int(old_to_new[p])
    for child, parent in enumerate(new_parents.tolist()):
        if int(parent) >= child:
            raise ValueError(f"row contract parent order invalid at child={child} parent={parent}")

    return RowSkeletonContract(
        raw_count=n,
        raw_order=[int(x) for x in raw_order],
        parents=new_parents,
        names=[name_list[int(i)] for i in raw_order],
        kept_raw_indices=[int(x) for x in raw_order],
        dropped_raw_indices=sorted(int(x) for x in drop),
        active_raw_indices=[int(i) for i in np.flatnonzero(active).tolist()],
        dropped_tail_leaf_indices=sorted(int(x) for x in drop),
    )
