...
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/clean_module_full_rebuild_20260704.log"
exec > >(tee -a "${LOG}") 2>&1

echo "clean_module_full_rebuild_start $(date -Is)"
...
export EVOWEAVE_REBUILD_ROOT=/ssdwork/liuhaohan/evorig/evoweave_rebuild_clean_full_20260704
export EVOWEAVE_UNIRIG_ROOT=/ssdwork/liuhaohan/evorig/evoweave_repo/external/UniRig

export EVOWEAVE_REBUILD_LIMIT=0
export EVOWEAVE_TEXVERSE_PASS0_WORKERS=16
export EVOWEAVE_TEXVERSE_PASS1_WORKERS=16
export EVOWEAVE_OXL_PASS1_WORKERS=16
export EVOWEAVE_STRICT_WORKERS=64
export EVOWEAVE_ROOTLESS_WORKERS=64
export EVOWEAVE_AUDIT_WORKERS=64
...
LOCK_FILE="${LOCK_DIR}/clean_module_full_rebuild_20260704.lock"
mkdir -p "${LOCK_DIR}"
exec 9>"${LOCK_FILE}"
flock 9

if [[ -f "${EVOWEAVE_REBUILD_ROOT}/rootless_clean/rootless_validation.json" \
   && -f "${EVOWEAVE_REBUILD_ROOT}/quality_distributions/rootless_alignment_candidates/rootless_alignment_report.md" ]]; then
  echo "clean_module_full_rebuild_already_complete ${EVOWEAVE_REBUILD_ROOT}"
  exit 0
fi
...
echo "clean_module_full_rebuild_end $(date -Is)"
