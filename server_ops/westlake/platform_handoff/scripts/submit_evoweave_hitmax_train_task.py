...
DEFAULT_GROUP_ID = "7acb9ff7-7b0f-4363-8e42-ec3521c32610"
DEFAULT_CARD_TYPE = "NVIDIA-A100-SXM4-40GB"
DEFAULT_COMMAND = (
    "bash /ssdwork/liuhaohan/jobs/evoweave_hitmax_fix_20260628/"
    "run_explicit_tree_gpr_train_20260628.sh"
)
DEFAULT_LOG_DIR = ""
...
def recent_tasks(base_url: str, session, page_size: int = 5) -> list[dict]:
    data, _ = request_json(
        session,
        "GET",
...
def main() -> int:
    ...
    parser.add_argument(
        "--name",
        default="evoweave_hitmax_explicit_tree_gpr_" + datetime.now().strftime("%Y%m%d_%H%M%S"),
    )
    ...
    if not args.submit:
        print("\nDry run only. Add --submit to create the training task.")
        return 0
